...
Wymagany jest [htmlq](https://github.com/mgdm/htmlq).

Pobieramy wszystkie linki do zgromadzonych próbek profilera - `curl -L --silent localhost:4200/_profiler/search?limit=100 | htmlq --attribute href '#search-results td:last-child a'`

Następnie możemy odpytać o szczytowe zużycie pamięci - `curl --silent localhost:4200/<SCIEZKA>?panel=time | htmlq --text 'div.metrics > div.metric > .value'`
...
function executeHtmlq($html, ...$arguments): array {
    ...
    $process = proc_open(implode(' ', ['htmlq', ...$arguments]), $descriptors, $pipes);
...
function main($baseUrl): void {
    $searchProfilerUrl = http_build_url($baseUrl, ['path' => '/_profiler/search', 'query' => http_build_query(['limit' => 100])]);
    $htmlBody = fetchWebpageHTML($searchProfilerUrl);
    $probesUrl = executeHtmlq($htmlBody, "--attribute", "href", "#search-results td:last-child a");

    foreach ($probesUrl as $path) {
        $url = http_build_url($baseUrl, ['path' => $path]);
        $data = extractDataFromProbe($url);
        displayData($data);
    }
}
